...
function create(repoRoot) {
	...
	function log(event) {
		...
		// Every line is timestamped. Client-supplied ts (browser clock) is
		// kept when sane; anything missing/garbage gets server time.
		const now = new Date().toISOString();
		if (typeof rec.ts !== 'string' || isNaN(Date.parse(rec.ts))) rec.ts = now;
	...
	function attachProcess(sourceName) {
		...
		console.error = function (...args) {
			try {
				...
				// Node lifecycle noise (deprecation warnings etc.) is not an
				// incident; keep it on stderr only.
				if (!/^\(node:/.test(msg)) {
